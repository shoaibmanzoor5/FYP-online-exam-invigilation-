...
class _CameraPageState extends State<CameraPage> {
  ...
  Future<void> receiveremovestudent() async {
    String remove;
    while (wrns) {
      var response = await http.get(
        Uri.parse('http://' + MyApp.ipaddress + '/amIRemoved/' + widget.id),
      );
  ...
  Future<String> SubmitPaper() async {
    ...
    for (int i = 0; i < length; i++) {
      var Ans = "A";
      if (selectA[i] == true) Ans = "A";
      if (selectB[i] == true) Ans = "B";
      if (selectC[i] == true) Ans = "C";
      if (selectD[i] == true) Ans = "D";

      var response = await http.post(
          Uri.parse('http://' +
              MyApp.ip +
              '/invigilation/api/std/uploadMCQBYStudent'),
          body: {
